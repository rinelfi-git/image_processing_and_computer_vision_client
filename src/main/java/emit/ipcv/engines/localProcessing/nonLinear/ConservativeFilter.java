 ...
 * @author rinelfi
 */
public class ConservativeFilter extends NonLinearLocalTransformation {

  public ConservativeFilter(int[][] image) {
    this.image = image;
  }
    ...
    for (int line = 0; line < lineLength; line++) {
      for (int column = 0; column < columnLength; column++) {
        if ((line - filterLineAxis) < 0 || (column - filterColumnAxis) < 0) {
          ...
          operation[line][column] = new IntArrayHelper2D(grayscaleSlice).getConservativeValue();
        }
      }
    }
    return operation;
