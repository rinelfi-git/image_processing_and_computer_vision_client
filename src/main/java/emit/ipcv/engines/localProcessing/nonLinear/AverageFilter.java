 ...
 * @author rinelfi
 */
public class AverageFilter extends NonLinearLocalTransformation {

  public AverageFilter(int[][] image) {
    this.image = image;
  }
    ...
    for (int line = 0; line < lineLength; line++) {
      for (int column = 0; column < columnLength; column++) {
        if ((line - filterLineAxis) < 0 || (column - filterColumnAxis) < 0) {
          ...
          float average = new IntArrayHelper2D(grayscaleSlice).getAverage();
          operation[line][column] = (average + 0.5f) >= (average + 1f) ? (int) Math.ceil(average) : (int) Math.floor(average);
        }
      }
    }
    return operation;
